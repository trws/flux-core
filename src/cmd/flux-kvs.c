...
void cmd_getobj (flux_t h, int argc, char **argv)
{
    ...
    if (!(rpc = content_load (h, key, sizeof (key))))
        err_exit ("content_load");
    if (content_load_get_data (rpc, &data, &size) < 0)
        err_exit ("content_load_get_data");
...
void cmd_putobj (flux_t h, int argc, char **argv)
{
    ...
    if ((size = read_all (STDIN_FILENO, &data)) < 0)
        err_exit ("read_all");
    if (!(rpc = content_store (h, data, size)))
        err_exit ("content_store");
    if (content_store_get_hash (rpc, &key, &key_size) < 0)
        err_exit ("content_store_get_hash");
    if (key_size != 20)
