...
#define FOP_REGISTER(CLASS, SELECTOR, STRUCT, MEMBER)          \
    fop_register_method (CLASS, (fop_vv_f)SELECTOR, #SELECTOR, \
                         offsetof (STRUCT, MEMBER));
#define FOP_REGISTER_CHECKED(CLASS, SELECTOR, STRUCT, MEMBER)                \
    fop_register_method (CLASS, (fop_vv_f) (((STRUCT){}).MEMBER = SELECTOR), \
                         #SELECTOR, offsetof (STRUCT, MEMBER));
int fop_register_method (fop_class_t *c,
                         fop_vv_f selector,
                         const char *name,
                         size_t offset);
int fop_implement_method_by_name (fop_class_t *c, const char *tag, fop_vv_f method);
